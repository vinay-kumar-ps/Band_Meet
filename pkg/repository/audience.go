...
func (u *UserRepo) UserSignup( userSignup models.UserSignup) (models.UserLoginResponse,error){
	...
	err := u.DB.Exec("INSERT INTO user_profile(username,name,email,phone,password)Values(?,?,?,?,?)RETURNING(username,id,email,phone)",userSignup.UserName,userSignup.Name,userSignup.Email,userSignup.Phone,userSignup.Password).Scan(&userLoginResponse).Error
	if err !=nil{
